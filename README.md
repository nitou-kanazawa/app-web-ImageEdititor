...
### テスト（Playwright）
見た目・操作・ピクセル変化までブラウザ自動操作で検証します。

```bash
npm install                      # 依存をインストール
npx playwright install chromium  # 初回のみブラウザ本体を取得
npm test                         # E2E + スクリーンショットを実行
```

- 静的サーバー（`python3 -m http.server`）はテスト実行時に自動起動します
- スクリーンショットは `test-results/` に出力されます（PC/モバイル）
- CIでは push / PR 時に GitHub Actions（`.github/workflows/test.yml`）で自動実行されます
